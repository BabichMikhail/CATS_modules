...
const _OK = 0;
      ...
      _PC1RANGE = 20;
               ...
               {_PCxBASE - база PC (AllowPC = x)}
               {_PCxRANGE - длина интервала для PC (AllowPC = x)}
...
begin
  ...
  case res of
    ...
    else   write ('* Неожиданный поворот * ');
  end;
...
BEGIN {€Ќ€–€Ђ‹€‡Ђ–€џ}
   if (ParamCount < 3) or (ParamCount>4) then
      Quit (_fail, '‘Ё­в ЄбЁб: <INPUT-FILE> <OUTPUT-FILE> <ANSWER-FILE> [<log-file>]');
